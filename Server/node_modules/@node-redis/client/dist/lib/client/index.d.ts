...
declare type ConvertArgumentType<Type, ToType> = Type extends RedisCommandArgument ? (Type extends (string & ToType) ? Type : ToType) : (Type extends Set<infer Member> ? Set<ConvertArgumentType<Member, ToType>> : (Type extends Map<infer Key, infer Value> ? Map<Key, ConvertArgumentType<Value, ToType>> : (Type extends Array<infer Member> ? Array<ConvertArgumentType<Member, ToType>> : (Type extends Record<keyof any, any> ? {
...
export declare type RedisClientType<M extends RedisModules, S extends RedisScripts> = RedisClient<M, S> & WithCommands & WithModules<M> & WithScripts<S>;
